...
|---|---|---|
| `customerId` | `customer_id` (PK, varchar 36) | **Đã chốt**: dùng thẳng `customer_id` hiện có trong DB làm định danh xuyên suốt (dạng mã nghiệp vụ, vd `KH001`) — không đổi sang UUID. `customer_code` (cột riêng, UNIQUE) không cần API trả về/FE dùng nữa vì trùng giá trị với `customer_id` trong dữ liệu hiện tại. |
...
```json
{
  "customer": { "...": "giống object AdminCustomer ở mục 2.1" },
...
- `coordinator`: map tạm từ `orders.created_by → users.full_name` — đây là "người tạo đơn", không hẳn là
  "người phụ trách" lâu dài theo đúng nghĩa; giữ tạm cho tới khi có khái niệm phụ trách riêng (ngoài
  phạm vi quyết định lần này, không thuộc nhóm bị loại ở mục 3 vì đây là dữ liệu cấp Order chứ không
  phải cấp Customer).
